...
运行示例
以 provider 示例为例，演示服务端启动、客户端调用流程：
1. 启动 ZooKeeper 服务
# 本地测试启动 ZooKeeper
...
2. 启动 RPC 服务端（服务提供者）
cd MPRPC/bin
./provider -i conf.json # 启动好友服务，默认监听 0.0.0.0:8000
...
3. 协议通信流程
客户端通过 Protobuf 定义的 Stub 类封装请求参数，序列化后发送给服务端
服务端接收数据后，通过 Protobuf 反序列化得到请求对象
服务端调用对应的业务实现（重写的虚函数），处理完成后构造响应对象
响应对象序列化后返回给客户端，客户端反序列化得到结果
二次开发指南
...
步骤 2：生成 Protobuf 代码
执行以下命令，生成 C++ 版本的协议代码（.pb.cc 和 .pb.h）：
...
步骤 6：编译运行
修改 CMakeLists.txt，添加新服务的编译规则（参考 example/ 目录下的配置），重新执行 ./autobuild.sh，然后启动服务端和客户端即可。
项目结构说明
MPRPC/
├── src/                  # 框架核心代码
...
配置文件说明
配置文件默认名为 conf.json，放置在项目bin目录，支持以下参数：
{
  "rpcserverip": "127.0.0.1", #服务端监听 IP
  "rpcserverport": 8100,      #服务端监听端口
  "zookeeperip": "127.0.0.1", #zookeeperip
  "zookeeperport": 2181       #ZooKeeper 服务端口
}
...
Q2：客户端调用失败，提示「服务未找到」？
A2：
检查 ZooKeeper 服务是否启动（zkServer.sh status）
检查服务端是否成功注册服务（查看服务端日志）
确认客户端配置的 ZooKeeper 地址与服务端一致
Q3：日志未输出或输出异常？
A3：
检查配置文件的 loglevel 是否正确（如设置为 ERROR 则不输出 INFO 日志）
确认日志目录有写入权限（默认输出到控制台，可修改 logger.cc 改为文件输出）
Q4：高并发场景下出现性能瓶颈？
A4：
调整 muduo 线程池大小（在 MPRpcProvider::Run() 中修改 server.setThreadNum(8)）
优化业务逻辑，避免在 RPC 方法中执行耗时操作
增加服务节点，通过 ZooKeeper 实现负载均衡
欢迎参与 MPRPC 项目的开发与优化，贡献流程如下：
Fork 本仓库到自己的 GitHub 账号
基于 dev 分支创建 Feature 分支（命名格式：feat/功能名称 或 fix/BUG描述）
提交代码时，遵循 Google C++ 编码规范
Commit 信息格式：类型：描述（如 feat：新增配置热加载功能、fix：修复 ZooKeeper 重连bug）
提交 PR 到本仓库的 dev 分支，PR 描述需说明功能或修复内容
等待维护者审核，通过后合并到主分支
许可证
本项目基于 MIT 许可证 开源，允许个人和商业项目自由使用、修改和分发，无需支付授权费用。
维护者
作者：ValarIMMORTAL
